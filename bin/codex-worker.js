...
import { loadSecurityConfig, assertWithinAllowedDirs } from "./security.js";
...
然后只回一句「这条属于 Group Relay 的需求，已记为工单：<标题>，会走反馈队列统一实现」。
判据同上：改客户端/桥接/MCP 本身的行为、抱怨软件毛病、要新端或改协议字段都算；
...
async function askCodex(config, messages) {
  ...
  // 目录白名单打开后,trusted 执行的工作区必须落在允许清单内,否则拒绝执行。
  // 关闭时(默认)直接放行,行为与历史一致。restricted 模式跑在临时 workerDir,不受此限。
  if (trustedExecution) {
